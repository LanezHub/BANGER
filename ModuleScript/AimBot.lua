...
local function IsPlayerVisible(Target)
	local PlayerCharacter = GetChar(Target)
	local LocalPlayerCharacter = GetChar(player)
	if not (PlayerCharacter or LocalPlayerCharacter) then return end 
	local PlayerRoot = FindFirstChild(PlayerCharacter, config.aimbot.aimPart1) or FindFirstChild(PlayerCharacter, "HumanoidRootPart")
...
RunService:BindToRenderStep("TemporalMainLoop", 1, function(deltaTime)
	...
	if aimbot.enabled and char then
		if aiming and target then
			...
			local partbot = GetPlayerPosPart(target)
			if pos and partbot then
				local cf1 = CFrame.new(camera.CFrame.p, partbot.CFrame.p) * CFrame.new(0, 0, -4)
				local r1 = Ray.new(cf1.p, cf1.LookVector * 1000)
				local obj, pos2 = game.Workspace:FindPartOnRayWithIgnoreList(r1,  {char.Head})
				if obj:IsA("BasePart") then 
					if config.aimbot.ignoreWalls and not IsPlayerVisible(target) then 
						target = nil
					else
	...
	fov_circle.Visible = config.aimbot.enabled and config.aimbot.showRange
	fov_circle.Radius = config.aimbot.range
end)

spawn(function()
	while not scriptTerminated do wait(0.001)
		UpdateChams()
		--RunService.RenderStepped:Wait()
	end
end)
	...
	while true do wait(0.001)
		...
		if aimbot.enabled and aiming then
			if target then
				...
				elseif config.aimbot.ignoreWalls and not IsPlayerVisible(target) then 
					target = nil 
				else
					target = nil
				end
				...
				for i, plr in pairs(Players:GetPlayers()) do
					if plr ~= player and IsEnemyAimbot(plr) and not IsDead(plr) then
						...
						if pos then 
							if config.aimbot.ignoreWalls and not IsPlayerVisible(plr) then return end
